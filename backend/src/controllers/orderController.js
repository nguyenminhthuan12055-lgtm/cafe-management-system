...
const orderController = {
  ...
  updateOrderStatus: async (req, res) => {
    try {
      ...
      res.json({ status: "success", message: "Cập nhật trạng thái hóa đơn thành công!", data: order });
    } catch (err) {
      res.status(400).json({ status: "error", message: err.message });
    }
  },

  // ✅ Thống kê doanh thu chi tiết theo Ngày / Tuần / Tháng — ?period=day|week|month
  getRevenueReport: async (req, res) => {
    try {
      const period = req.query.period || "day";
      const stats = await orderService.getRevenueReport(period);
      res.json({ status: "success", data: stats, period });
    } catch (err) {
      res.status(500).json({ status: "error", message: err.message });
    }
  },

  // ✅ Tổng quan nhanh: Tổng doanh thu / Doanh thu hôm nay / Tổng số hóa đơn
